    ...
    func uploadImage(_ image: UIImage, completion: @escaping (Result<String>) -> ()) {
        ...
        let operation = DownloadOperation(session: URLSession.shared, dataTaskURLRequest: request) { (data, response, error) in
            DispatchQueue.main.async {
                ...
                guard let mimeType = response.mimeType, mimeType == "application/json", let data = data, let json = try? JSONSerialization.jsonObject(with: data, options: []) as? NSDictionary, let result = json["data"] as? NSDictionary, let link = result["link"] as? String else { return completion(.error("Error getting data from the response")) }
